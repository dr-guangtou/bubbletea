"""Execute radial UCD search using the frozen Gaia selector and local background.

Performs concentric annulus queries around target galaxies, applies
the versioned point-source selection, and identifies statistical excesses
...
def build_query(ra: float, dec: float, radius_deg: float) -> str:
    ...
    return f"""
    SELECT
        {", ".join(RAW_GAIA_COLUMNS)}
    FROM gaiadr3.gaia_source
    WHERE 1 = CONTAINS(
        POINT('ICRS', ra, dec),
        CIRCLE('ICRS', {ra}, {dec}, {radius_deg})
    )
    AND phot_g_mean_mag BETWEEN {MINIMUM_G_MAGNITUDE} AND {MAXIMUM_G_MAGNITUDE}
    """


def apply_frozen_selector(df: pd.DataFrame) -> pd.DataFrame:
    """Apply the frozen selector without legacy query or post-score cuts."""
    if df.empty:
        return df
    scored = load_frozen_selector().score(df)
    return scored.loc[scored["point_source_logistic_selected"]].copy()
...
def search_around_galaxy(target: dict) -> dict | None:
    ...
    try:
        ...
        df_cand = apply_frozen_selector(df_raw)
...
def main(n_limit: int | None = None, min_glat: float = 30.0):

...
from scripts.config import GAIA_CROSSMATCH_AUDIT, GAIA_CROSSMATCH_EXPORT
...
def build_gaia_query(ra: float, dec: float, radius_arcsec: float) -> str:
    """Build a Gaia DR3 service-side cone query."""
...
def xmatch_gaia_datalab(
    objects: list[dict[str, Any]], radius_arcsec: float = 1.0
    ...
    for i, obj in enumerate(objects):
        ...
        try:
            job = service.run_sync(build_gaia_query(obj["ra"], obj["dec"], radius_arcsec))
...
def main():
    ...
    df_gaia, df_audit = xmatch_gaia_datalab(objects)

    if not df_gaia.empty:
        update_db_with_gaia(conn, df_gaia)
        df_gaia.to_csv(GAIA_CROSSMATCH_EXPORT, index=False)
        logger.info("Exported Gaia matches to %s", GAIA_CROSSMATCH_EXPORT)

    GAIA_CROSSMATCH_AUDIT.parent.mkdir(parents=True, exist_ok=True)
    df_audit.to_csv(GAIA_CROSSMATCH_AUDIT, index=False)
    logger.info("Exported Gaia cross-match audit to %s", GAIA_CROSSMATCH_AUDIT)
